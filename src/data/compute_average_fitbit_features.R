...
fitbit_features <- read_csv(here::here("balance-quantitative", input_file_path, input_file_name))
...
average_fitbit_features <- fitbit_features %>%
  ...
  summarize(
    ...
    n_days = n(),
    n_days_averaged = sum(sufficient_fitbit_data),
    across(
...
write_csv(average_fitbit_features, file = here::here("balance-quantitative", output_file_path, output_file_name))
